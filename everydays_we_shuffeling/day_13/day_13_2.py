def find_mirror(grid_lines):
    for index in range(1, len(grid_lines)):  # index is the mirror
        ...
        c = sum(above[i][index] != below[i][index] for i in range(len(above)) for index in
                range(len(above[i])))  # c  = for every right thing
...
def main(file):
    ...
    for block in file:
        ...
        total += 100 * find_mirror(grid_lines) or find_mirror(list(zip(*grid_lines)))
    print(total)
