...
def level(bing):
    ...
    for i in range(len(lines)):  # iterate over rows
        row = lines[i]
        temp = ""
        sec_temp=""
        flag = 0

        for j in range(len(row)):  # iterate over characters in the row
            if row[j].isdigit():  # check if the character is a digit
                if flag == 0:
                    temp += row[j]
                    flag += 1
                elif flag == 1:
                ...
                for key in num_mapping:
                    if substring.startswith(key):
                        if flag == 0:
                            temp = str(num_mapping[key])
                            flag += 1
                        else:
                            sec_temp = str(num_mapping[key])

        if sec_temp == "":
            total_sum += int(temp + temp)
        else:
            total_sum += int(temp + sec_temp)
